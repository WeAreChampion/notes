...
void isRightStr(string str)
{
	...
	{
		DispStack();
		DispStr(str,i);//disp Str from i
		string topString=PopStr();//the stack Pop
		if(str[i]==topString[0])
		{
			cout<<setiosflags(ios::left)<<str[i]<<"匹配"<<endl;
			i++;
			continue;
		}
		int pos=findLLTable(topString);
		...
		cout<<topString<<"->"<<startString<<endl;
	}
}
*/